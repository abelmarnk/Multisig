...
use crate::{state::{
    ProposalTransaction, group::Group, error::TokenError
}};
...
pub fn execute_proposal_transaction_handler(
    ...
    require_gte!(Clock::get()?.unix_timestamp,
        proposal_transaction.get_valid_from(), TokenError::TransactionNotRipe);
    ...
    let mut signer_seeds: Vec<Vec<&[u8]>> = Vec::with_capacity(proposal_transaction.asset_indices.len());
        
    for asset_index in proposal_transaction.asset_indices.iter() {
        // Find the matching ProposalAsset from the proposal_transaction
            
        let asset_key = &proposal_transaction.instruction.
            accounts[usize::from(*asset_index)].key;
    ...
    let signer_slices: Vec<&[&[u8]]> = signer_seeds.iter().
